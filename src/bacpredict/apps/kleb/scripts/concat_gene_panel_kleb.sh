...
#SBATCH --output=kleb_concat_panel_%A_%a.out
#SBATCH --error=kleb_concat_panel_%A_%a.err
#SBATCH --array=0-21
...
CMP=$D/train_kleb_ast/pangena_predict/esm_vs_ft_per_gene/$DRUG/esm_vs_ft_per_gene_${DRUG}.csv
OUT=$D/train_kleb_ast/pangena_predict/concat_gene_panel/$DRUG
mkdir -p "$OUT"
...
uv run python src/kleb_ast/concat_gene_panel_kleb.py \
