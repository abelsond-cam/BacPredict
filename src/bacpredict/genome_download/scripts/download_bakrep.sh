...
# Change to project root so uv can find pyproject.toml and the bacpredict package
...
micromamba run -n bakrep_download python /home/dca36/workspace/BacPredict/src/bacpredict/genome_download/scripts/collect_bakrep_samples.py \
    --metadata "$TSV_FILE" \
    --n "$N" \
    --filetype "$FILE_TYPE" \
    $SKIP_ARG \
...
if [ "$FILE_TYPE" = "gff3" ]; then
    ...
    echo "============================================"
    micromamba run -n bakrep_download python /home/dca36/workspace/BacPredict/src/bacpredict/genome_download/scripts/flatten_klebsiella_gff3.py
fi
