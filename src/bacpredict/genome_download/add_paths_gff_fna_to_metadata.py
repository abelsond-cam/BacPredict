...
def main(argv: list[str] | None = None) -> None:
    """CLI entry point. Usage: python -m bacpredict.genome_download.add_paths_gff_fna_to_metadata [optional_metadata_tsv]."""
    if argv is None:
        argv = sys.argv[1:]
    if len(argv) > 1:
        raise SystemExit(
            "Usage: python -m bacpredict.genome_download.add_paths_gff_fna_to_metadata [optional_metadata_tsv]"
        )
