...
| `kp_eval_2026-06/` | a full Kp eval directory — `eval_summary.csv` plus three rendered figures | **The wrong numbers under a different filename.** Its `auroc` column reads colistin **0.8072**, azithromycin **0.8268**, tetracycline **0.9139** (of record 0.9094 / 0.7993 / 0.8694), and its `n_eval` is the **deprecated May cohort** (ertapenem 418 vs 424, colistin 281 vs 282, azithromycin 355 vs 384). It survived the first quarantine because that pass globbed `*amr_summary_panel*` and this file is not called that |
