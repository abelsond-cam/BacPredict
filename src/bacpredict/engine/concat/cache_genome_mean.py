"""GPU — cache the frozen Bacformer genome-mean per sample (drug-agnostic), for the CPU concat sweep.
...
Organism-agnostic: the cohort is defined by an AST sheet; the thin ``apps/<organism>`` CLI supplies the
data-root defaults.
"""
...
def load_sample_ids(ast_sheet: Path) -> list[str]:
    """Sorted unique Sample IDs from the AST sheet (drug-agnostic — every labelled genome)."""
...
def run(*, ast_sheet: Path, esm_store_dir: Path, output_npz: Path, device: str,
        max_samples: int | None = None) -> None:
    """Cache the frozen Bacformer genome-mean over the AST-sheet cohort to ``output_npz``."""
    sample_ids = load_sample_ids(ast_sheet)
    if max_samples is not None:
        sample_ids = sample_ids[:max_samples]
    logger.info("Caching frozen Bacformer genome-mean over %d genomes on %s", len(sample_ids), device)

    mean_mat, kept = compute_genome_means(sample_ids, esm_store_dir, device=device)
    if not kept:
...
def main() -> None:
    """Generic CLI (organism apps wrap this with their data-root defaults)."""
    ...
    run(ast_sheet=args.ast_sheet_path, esm_store_dir=args.esm_store_dir, output_npz=args.output_npz,
