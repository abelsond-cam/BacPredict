...
:mod:`pangena_predict.bacformer_genome_vectors` — same frozen-forward + flat-index pattern,
...
- **D2 — ``--checkpoint-dir``:** probe a *fine-tuned* backbone's ``.bacformer`` encoder instead of
  ...
  pool obliterates it (companion to :mod:`pangena_predict.head_pool_attention_probe`, which measures
