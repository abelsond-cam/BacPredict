...
#
#   prep    (himem, 16c/64G/6h)        : sublineage clusters + prime LMM cache + split matrix -> N chunks
#   array   (himem, ${CPU}c/48G/24h)   : --array=0..N-1, pyseer per chunk reusing the cache (--load-lmm)
#   combine (himem, 8c/48G/3h)         : cat .assoc + union patterns -> pyseer_postprocess --feature-mode unitigs
#
...
PREP=$(sbatch --parsable --account=$ACCT --partition=icelake-himem --nodes=1 --ntasks=1 \
    --cpus-per-task=16 --mem=64G --time=6:00:00 --job-name="uprep_$PAIR" \
    --export=ALL,PHASE=prep "$JOB")
echo "prep    : $PREP"

ARRAY=$(sbatch --parsable --account=$ACCT --partition=icelake-himem --nodes=1 --ntasks=1 \
    --cpus-per-task="$CPU" --mem=48G --time=24:00:00 --array=0-$((NSHARDS-1)) \
...
COMBINE=$(sbatch --parsable --account=$ACCT --partition=icelake-himem --nodes=1 --ntasks=1 \
    --cpus-per-task=8 --mem=48G --time=3:00:00 --dependency=afterok:"$ARRAY" \
