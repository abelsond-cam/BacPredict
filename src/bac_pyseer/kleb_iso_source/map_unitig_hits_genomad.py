...
# The generic placement + select engine (classifier-agnostic; NOT geNomad-specific) lives in
# unitig_placement — this module adds only the geNomad classifier layer on top.
try:  # package import when bac_pyseer is on the path (editable install)
    ...
    from bac_pyseer.kleb_iso_source.unitig_placement import run_select as _generic_select
    ...
    from unitig_placement import run_select as _generic_select  # type: ignore[no-redef]
...
# geNomad classification tags → class names (priority PLASMID > VIRUS > ASM-only). The _TAG_* symbols
# and the placement/IO/sub-matrix helpers now live in unitig_placement (imported above).
...
# --------------------------------------------------------------------------------------------------
# geNomad classification (build_automaton + scan_carrier now live in unitig_placement)
# --------------------------------------------------------------------------------------------------
...
def run_select(args: argparse.Namespace) -> None:
    """``select``: the generic id_map + sub-matrix + carrier resolution, then the geNomad layout probe.

    The generic engine lives in :func:`unitig_placement.run_select`; here we add the geNomad-specific
    per-sample layout probe and record it in the manifest.
    """
    mani = _generic_select(args)
    out = args.out_dir
    carrier_union = pd.read_csv(out / "carriers.resolved.tsv", sep="\t")["Sample"].astype(str).tolist()
    ...
    mani["genomad_layout_found"] = sum(1 for p in probe if p["found"])
    (out / "select_manifest.json").write_text(json.dumps(mani, indent=2))
...
# _rollup_pattern_group + _rollup_overall now imported from unitig_placement.
