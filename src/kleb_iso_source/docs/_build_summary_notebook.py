...
CELLS = [
    ...
    code(
        "lb = json.loads((COHORT_DIR / 'linear_baselines.json').read_text())",
        "rows = []",
    ...
    md(
        "**Linear-baseline AUROC vs Bacformer, across the three cohorts:**",
        "",
        "| Cohort | Bacformer | country+SL baseline | gap (Bacformer − baseline) |",
        "|---|---|---|---|",
        "| `all_samples` | 0.827 | **0.841** | **−0.014** ⚠ country shortcut |",
        "| `sampled_country_2_1_stratified` | 0.762 | 0.675 | **+0.087** |",
        "| **`sampled_country_2_1_all`** (headline) | **0.786** | 0.694 | **+0.092** |",
    ),
    md(
