...
#
# Two group columns x two split scopes = four tables:
#   Sublineage    x {evaluate, all}
#   Clonal group  x {evaluate, all}       <- note the SPACE in the column name
#
# WHY BOTH SCOPES. The holdout alone leaves only 5 sublineages / 4 clonal groups at n>=100, which is
# too few to say the signal holds *within* a clone. Scoring the whole cohort lifts that to 20 / 15.
# But the train rows were fitted on, so `all` AUROCs are NOT measurements — read them for the PATTERN
# across groups (does every clone separate?), and quote only the `evaluate` numbers. stratified_metrics
# stamps `split_scope` into every row and warns loudly when train rows are included.
#
