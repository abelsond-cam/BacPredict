...
Both feed linear probes in :mod:`snp_embeddings.snp_vs_esm_prediction` (``bacformer_gene_token`` /
``bacformer_mean``) and the concat driver
:mod:`snp_embeddings.concatenate_bacformer_genome_esm_protein_emb` (genome mean ⊕ ESM gene vector).

Indexing
--------
The gene token sits at the flat-protein index from the gene-presence table
(:func:`snp_embeddings.locate_gene.build_gene_presence_table`): the real proteins of the stored
...
def _extract_gene_token_and_mean(
    ...
    *,
    ...
    :func:`snp_embeddings.locate_gene.build_gene_presence_table`). Returns ``(token [N, dim],
