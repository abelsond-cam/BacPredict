...
| ------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| **Step 1:** `src/bacpredict/sample_and_label/preprocess_ebi_amr_records.py` (calls `src/bacpredict/sample_and_label/convert_ast_data.py`) | Preprocess EBI AST records and pivot to sample-level tables                         | Raw EBI AMR CSV; min-antibiotic threshold                                                         | `binary_ast.csv`, metadata CSV, `regression_log_mic.csv`, antibiogram                                              |
| **Step 2:** `src/bacpredict/sample_and_label/prepare_esmc_embeddings_and_labels_to_finetune_amr.py`                                        | Add train/validate/evaluate splits and merge AST labels into per-sample `.pt` files | `binary_ast.csv`, embedding files `{Sample}_esm_embeddings.pt`                                    | `binary_ast_with_split.csv`, `ast_training/{train,validate,evaluate}/{Sample}_with_ast.pt`, missing-samples report |
| **Step 3:** `src/bacpredict/train/train_amr.py` (typically via `src/bacpredict/train/scripts/train_on_slurm_amr.sh`)                           | Fine-tune Bacformer for one selected antibiotic label                               | `ast_training/train`, `ast_training/validate`, `binary_ast_with_split.csv`, `--drug <antibiotic>` | Fine-tuned checkpoints and training logs                                                                           |
...
```bash
uv run python src/bacpredict/sample_and_label/stratified_isolation_source_sampling.py \
  --isolation-sources blood respiratory
```

**2. Prepare merged `.pt` files** (`prepare_esmc_embeddings_and_labels_to_finetune_isolation_source.py`). This step reads many embedding files and is **too slow for a typical login node** at scale (roughly five minutes per thousand genomes). On the cluster, use the CPU Slurm wrapper: edit `isolation_sources="blood respiratory"` in `src/bacpredict/sample_and_label/scripts/prepare_iso_source_data_for_training.sh` (same idea as `src/bacpredict/sample_and_label/scripts/cpu_slurm.sh`), then submit:

```bash
sbatch src/bacpredict/sample_and_label/scripts/prepare_iso_source_data_for_training.sh
```
NOTE:  You must edit this script to include the isolation sources you want to train on (not CLI)

**3. Fine-tune Bacformer** on a GPU node. Edit `isolation_sources` in `src/bacpredict/train/scripts/train_isolation_source.sh` so it matches the pair you prepared, then:

```bash
sbatch src/bacpredict/train/scripts/train_isolation_source.sh
```
NOTE:  You must edit this script to include the isolation sources you want to train on (not CLI)
...
```bash
uv run python src/bacpredict/sample_and_label/prepare_esmc_embeddings_and_labels_to_finetune_isolation_source.py \
  --isolation-sources blood respiratory \
...
For Slurm-based prepare, add the same extra flags to the `uv run python ...` line inside `src/bacpredict/sample_and_label/scripts/prepare_iso_source_data_for_training.sh` (or run that command inside an interactive CPU allocation instead of on the login node). For training overrides (paths, learning rate, and so on), edit `src/bacpredict/train/scripts/train_isolation_source.sh` or call `train_isolation_source.py` with the same arguments from a GPU job.
